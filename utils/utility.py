...
def normalize(df , train = True, min_max_scaler = None): 
    ...
    return pd.DataFrame(x_scaled) , min_max_scaler


def process_MSL_SMAP(file):
    LOG = "../notebooks/data/MSL-SMAP/train/"+file+".npy"
    LOG_test = "../notebooks/data/MSL-SMAP/test/"+file+".npy"
    LABEL= "../notebooks/data/MSL-SMAP/labels/"+file+".txt"

    log= pd.DataFrame(np.load(LOG))
    label = open(LABEL, "r").read()
    log_test = pd.DataFrame(np.load(LOG_test))
    
    log['date'] = pd.date_range(start='1/1/2019', periods=len(log), freq='S')
    log =  log.resample('1S' , on="date").median()
    
    label = label.replace("[", "").replace("]"," ").replace(",","").split()
    
    target = [0] * log_test.shape[0]
    for i in range(0,len(label) , 2):
        for j in range(int(label[i]) - log_test.shape[0], int(label[i+1])- log_test.shape[0]): 
            target[j] = 1
    
        
    log_normalize, min_max_scaler = normalize(log)
    log_test_normalize , min_max_scaler = normalize(log_test , train = False, min_max_scaler = min_max_scaler)
    
    X_test = log_test_normalize.values
    y_test = target
    X_train= log_normalize.values
    
    return X_test , y_test, X_train


def pointAjust(truth, pred):
        res= 0
        flag = False
        flag2 = False
